...
function generate_code_layer_julia(symex, var, lorr, vors)
    ...
    code *= handle_input_args_julia(lorr, vors);
    code *= "\n";
    
    # If needed, compute derivative matrices
    if need_derivs
        code *= build_derivative_matrices_julia(lorr, vors);
    end
    code *= "\n";
    
    # Evaluate or fetch the values for each needed entity.
    code *= prepare_needed_values_julia(entities, var, lorr, vors);
    code *= "\n";
    
    # Form the final elemental calculation
    code *= make_elemental_computation_julia(terms, var, dofsper, offset_ind, lorr, vors);
...
#                 for ci=1:length(var[i].symvar.vals) # vector components
#                     # Process the terms for this variable
#                     for i=1:length(terms[vi])
#                         (term_result, test_ind, trial_ind) = generate_term_calculation(terms[vi][i], var, lorr);
...
#             terms = terms[1];
#             for ci=1:length(var.symvar.vals) # vector components
#                 # Process the terms for this variable
#                 for i=1:length(terms)
#                     (term_result, test_ind, trial_ind) = generate_term_calculation(terms[i], var, lorr);
...
# function generate_term_calculation(term, var, lorr)
#     result = "";
