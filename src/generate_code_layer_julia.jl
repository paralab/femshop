...
function handle_input_args_julia(lorr, vors)
    code = "";
    if vors == "volume"
        code *=
...
function build_derivative_matrices_julia(lorr, vors)
    code = 
"
...
function prepare_needed_values_julia(entities, var, lorr, vors)
    code = "";
...
function make_elemental_computation_julia(terms, var, dofsper, offset_ind, lorr, vors)
...
function generate_term_calculation_julia(term, var, lorr)
    result = "";
