            ...
            if vors == "volume"
                if length(entities[i].derivs) > 0
                    ...
                    for di=1:length(entities[i].derivs)
                        ...
                        code *= "Femshop.cachesim_load_range(7)"*"; # d/d"*xyzchar[entities[i].derivs[di]]*" of test function\n";
                    end
                    need_deriv_matrix = true;
                else
                    code *= "Femshop.cachesim_load_range(7)"*"; # test function.\n";
                end
            else
                if length(entities[i].derivs) > 0
                    xyzchar = ["x","y","z"];
        ...
        elseif is_unknown_var(entities[i], var) && lorr == LHS
            if vors == "volume"
                if length(entities[i].derivs) > 0
                    xyzchar = ["x","y","z"];
                    for di=1:length(entities[i].derivs)
                        code *= "Femshop.cachesim_load_range(7)"*"; # d/d"*xyzchar[entities[i].derivs[di]]*" of trial function\n";
                    end
                    need_deriv_matrix = true;
                else
                    code *= "Femshop.cachesim_load_range(7)"*"; # trial function.\n";
                end
            else
                if length(entities[i].derivs) > 0
                    xyzchar = ["x","y","z"];
            ...
            elseif ctype == 3 # a known variable value
                ...
                if vors == "volume"
                    ...
                    csarrayind = 17+cval;
                    code *= "Femshop.cachesim_load_range($csarrayind, loc2glb)";
                    ...
                    # If derivatives are needed, must evaluate at all volume nodes.
                    csarrayind = 17+cval;
                    code *= "Femshop.cachesim_load_range($csarrayind, loc2glb[1])";
                    if length(entities[i].derivs) > 0
