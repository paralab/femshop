        ...
        elseif is_unknown_var(entities[i], var) && lorr == LHS
            ...
            elseif ctype == 3 # a known variable value
                ...
                if vors == "volume"
                    ...
                    if length(entities[i].derivs) > 0
                        if dgside == 0
                            code *= cnameside1 * " = copy((Femshop.variables["*string(cval)*"]).values["*string(entities[i].index)*", loc2glb[1]]);\n";
                            code *= cnameside2 * " = copy((Femshop.variables["*string(cval)*"]).values["*string(entities[i].index)*", loc2glb[2]]);\n";
                        elseif dgside == 1
