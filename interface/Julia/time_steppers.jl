...
function lsrk4(Nsteps, dt, rhs)
    ...
    for ti=1:Nsteps
        for rki=1:5
            ...
            rhsv = rhs(variables, rktime);
            resu = rk4a[rki].*resu + dt.*rhsv;
            for vi=1:length(variables)
                sector = ((vi-1)*mesh_data.nel+1):vi*mesh_data.nel;
                variables[vi].values = variables[vi].values .+ rk4b[rki].*resu[:,sector];
            end
        end
        time += dt;
    end
